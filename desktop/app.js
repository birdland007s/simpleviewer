...
function createMenuTemplate(settings) {
    ...
    submenu: [
      {
        label: 'Help && &Support',
        click: function() {
          shell.openExternal('http://simpleview.com/help');
        },
      },
    ],
  };
    ...
    submenu: [
      {
        ...
        click(item, focusedWindow) {
          if (focusedWindow) {
            focusedWindow.webContents.send('appCommand', { action: 'newView' });
          }
        },
      },
      {
          ...
          if (focusedWindow) {
            dialog.showSaveDialog(
              focusedWindow,
              {
                ...
                defaultPath: path.join(app.getPath('desktop'), 'views.zip'),
              },
              filename =>
                focusedWindow.webContents.send('appCommand', {
  ...
  if (process.platform === 'darwin') {
    // Add the 'Simpleview' menu for os x
